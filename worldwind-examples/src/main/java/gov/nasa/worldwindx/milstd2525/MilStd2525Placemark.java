...
/**
 * This utility class generates Placemarks containing MILSTD2525C symbols.
 */
public class MilStd2525Placemark {
     ...
     *
     * @param position   The geographic position of the symbol.
     * @param symbolCode The MILSTD2525C symbol code.
     * @param modifiers  The MILSTD2525C modifiers. May be empty, but not null.
     *
     * @return A new Placemark representing the MILSTD2525C symbol.
     */
    public static Placemark fromSymbolCode(Position position, String symbolCode, SparseArray<String> modifiers) {
        return fromSymbolCode(position, symbolCode, modifiers, defaultAttributes);
    }
     ...
     * @return A new Placemark representing the MILSTD2525C symbol.
     */
    public static Placemark fromSymbolCode(Position position, String symbolCode, SparseArray<String> modifiers, SparseArray<String> attributes) {
        ImageInfo imageInfo = MilStd2525Placemark.renderImage(symbolCode, modifiers, attributes);
        return fromImageInfo(position, imageInfo);
    }

    /**
     * Creates a placemark from an ImageInfo object.
     *
     * @param position  The geographic position of the symbol.
     * @param imageInfo The object containing the symbol's bitmap and meta data.
     *
     * @return A new Placemark representing the MILSTD2525C symbol.
     */
    public static Placemark fromImageInfo(Position position, ImageInfo imageInfo) {

        // For demonstration and validation,add a leader line to above ground symbols.
        ...
        return new Placemark(position, attr, null);
    }
    ...
    public static ImageInfo renderImage(String symbolCode, SparseArray<String> modifiers, SparseArray<String> attributes) {
        if (!initialized) {
            throw new IllegalStateException("MilStd2525Placemark has not been initialized.");
        }
