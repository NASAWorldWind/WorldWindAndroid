...
class Render {
    ...
    static void RenderLonelyTriangles(GLUtessellatorImpl tess, GLUface f) {
        ...
        tess.callBeginOrBeginData(GLES10.GL_TRIANGLES);
    ...
    private static class RenderFan implements renderCallBack {
        public void render(GLUtessellatorImpl tess, GLUhalfEdge e, long size) {
             ...
             */
            tess.callBeginOrBeginData( GLES10.GL_TRIANGLE_FAN);
            tess.callVertexOrVertexData( e.Org.data);
    ...
    public static void __gl_renderBoundary(GLUtessellatorImpl tess, GLUmesh mesh) {
        ...
        for (f = mesh.fHead.next; f != mesh.fHead; f = f.next) {
            if (f.inside) {
                tess.callBeginOrBeginData( GLES10.GL_LINE_LOOP);
                e = f.anEdge;
                do {
                    tess.callVertexOrVertexData( e.Org.data);
                    e = e.Lnext;
...
//            CachedVertex vc;
        ...
        if ( !USE_OPTIMIZED_CODE_PATH ) {
            ...
            tess.callBeginOrBeginData( tess.boundaryOnly ? GLES10.GL_LINE_LOOP
                    : (tess.cacheCount > 3) ? GLES10.GL_TRIANGLE_FAN
                    : GLES10.GL_TRIANGLES);
